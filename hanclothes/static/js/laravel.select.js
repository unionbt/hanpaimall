...
$().ready(function(){
	...
	$('.select-model').each(function(){
		...
		var has_tip = $this.data('has_tip');
		var dd_text = $this.data('default_text');
		var dd_value = $this.data('default_value');
		$.POST($.baseuri + $this.data('model')+'/data/json', {all: 'true'}, function(json){
			var data = [];
			if (json.result == 'success') {
				var items = json.data.data;
				if(has_tip)
				    data.push({'id':dd_value?dd_value:0,'text':dd_text?dd_text:'无'});
