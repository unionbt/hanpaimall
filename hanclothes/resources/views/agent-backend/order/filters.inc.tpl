	...
	<input type="hidden" name="base" value="<{$_base}>">
		...
		<div class="col-md-9">
			<div class="input-group">
				<input type="text" id="id" name="filters[id][equal]" class="form-control" placeholder="请输入关键词..." value="<{$_filters.id.equal}>">
				
			</div>
		</div>
	</div>
	<div class="form-group col-sm-4">
		<label class="col-md-3 control-label" for="uid">用户昵称</label>
		<div class="col-md-9">
			<select name="filters[uid][in][]" class="suggest-model form-control" data-model="factory/agent-user" data-text="{nickname}" data-term="nickname" data-placeholder="请输入用户昵称" value="<{$_filters.uid.in|default:[]|implode:','}>" multiple="multiple"></select>
		</div>
	</div>
	<div class="form-group col-sm-4">
