...
<{block "datatable-columns-options"}>
		var columns_options = [{'data': null, orderable: false, 'render': function (data, type, full){
			return '<div class="btn-group">\
				...
				+ (parseInt(full['status']) ==1 ?'<a href="<{''|url}>/<{block "namespace"}>admin<{/block}>/<{block "name"}><{/block}>/'+full['id']+'/cancel" data-toggle="tooltip" method="delete" confirm="您确定取消这个订单：'+full['id']+'吗？" title="取消" class="btn btn-xs btn-info"><i class="fa fa-times"></i></a>':'')
