...
<form action="<{'admin'|url nofilter}>/<{block "name"}><{/block}>/" method="GET" class="form-bordered form-horizontal">
	...
	<div class="form-group col-sm-4">
		...
		<div class="col-md-9">
			<select name="filters[sid][in][]" id="sid" class="suggest-model form-control" data-model="admin/store" data-text="{name}" data-term="name" data-placeholder="请输入门店名字" value="<{$_filters.sid.in|default:[]|implode:','}>" multiple="multiple"></select>
		</div>
	</div>
