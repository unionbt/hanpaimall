...
var columns_plus = [
	...
	{'data': 'agent.name', orderable: false},
...
var columns_options = [{'data': null, orderable: false, 'render': function (data, type, full){
	return '<div class="btn-group">'
		+ (!full['audited'] ? '<a href="<{'admin'|url}>/store-audit/'+full['id']+'" method="put" confirm="您确定审核通过这个申请：'+full['username']+'吗？" data-toggle="tooltip" title="审核" class="btn btn-xs btn-default"><i class="fa fa-check"></i></a>' : '')
		+'<a href="<{'admin'|url}>/store-audit/'+full['id']+'" method="delete" confirm="您确定删除这个申请：'+full['username']+'吗？" data-toggle="tooltip" title="删除" class="btn btn-xs btn-danger"><i class="fa fa-times"></i></a></div>';
	}
}];
<{/block}>