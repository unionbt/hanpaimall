...
<form action="<{'admin'|url nofilter}>/<{block "name"}><{/block}>/" method="GET" class="form-bordered form-horizontal">
	...
	<div class="form-group col-sm-4">
		...
		<div class="col-md-9">
			<select name="filters[aid][in][]" id="aid" class="suggest-model form-control" data-model="admin/factory" data-text="{name}" data-term="name" data-placeholder="请输入代理商名字" value="<{$_filters.aid.in|default:[]|implode:','}>" multiple="multiple"></select>
		</div>
	</div>
	<div class="form-group col-sm-4">
