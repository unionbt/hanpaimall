<div class="form-group">
	...
	<div class="col-md-9">
		<select name="id" id="uid" value="<{$_data.id}>" class="form-control suggest-model" data-model="admin/member" data-text="{realname}({phone})" data-term="realname" data-filters='{"role_id": "5"}' data-placeholder="请输入真实姓名查找"></select>
...
<div class="form-group">
	...
	<div class="col-md-9">
		<select name="agent_ids[]" id="agent_ids" class="select-model form-control" value="<{if !empty($_data)}><{$_data->agent_ids()->toArray()|implode:','}><{/if}>" data-model="admin/agent" data-text="{name}" data-placeholder="请选择代理商名字" multiple="multiple"></select>
	</div>
</div>
<div class="form-group">
...
$().ready(function(){
	...
	$('#agent_ids').on('change', function(){
		...
		$.POST('<{'admin/agent/data/json'|url}>', {'filters[id][in]': value}, function(json){
