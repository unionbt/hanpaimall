...
<div class="content-header">
	<div class="header-section">
		<h1>
			<i class="hi hi-edit"></i><{block "title"}><{/block}>管理<br><small>修改<{block "title"}><{/block}>资料!</small>
		</h1>
	</div>
</div>
...
<div class="block-content">
	...
	<form action="<{''|url}>/<{block "namespace"}>admin<{/block}>/<{block "name"}><{/block}><{block "subaction"}><{/block}>/<{block "id"}><{$_data->getKey()}><{/block}>" method="POST" class="form-horizontal form-bordered" id="form">
		<{csrf_field() nofilter}>
