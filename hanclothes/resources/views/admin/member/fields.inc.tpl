...
<div class="form-group">
	...
	<div class="col-md-9">
		<select id="role_ids" name="role_ids[]" class="form-control select-model" value="<{if !empty($_data)}><{$_data->roles->modelKeys()|implode:','}><{/if}>" data-model="admin/role" data-text="{display_name}({name})" data-placeholder="请输入用户组" multiple="multiple"></select>
	</div>
</div>
