...
<form action="<{'admin'|url nofilter}>/<{block "name"}><{/block}>/" method="GET" class="form-bordered form-horizontal">
	...
	<div class="form-group col-sm-4">
		...
		<div class="col-md-9">
			<select name="filters[id]" id="uid" value="<{$_filters.id.equal}>" class="form-control suggest-model" data-model="admin/member" data-text="{realname}({phone})" data-term="realname" data-filters='{"role_id": "3"}' data-placeholder="请输入真实姓名查找"></select>
		</div>
	</div>
