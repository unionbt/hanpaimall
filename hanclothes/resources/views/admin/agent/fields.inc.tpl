<div class="form-group">
	...
	<div class="col-md-9">
		<select name="id" id="uid" value="<{$_data.id}>" class="form-control suggest-model" data-model="admin/member" data-text="{realname}({phone})" data-term="realname" data-filters='{"role_id": "4"}' data-placeholder="请输入真实姓名查找"></select>
...
<div class="form-group">
	<label class="col-md-3 control-label" for="factory_ids">所代理厂家</label>
	<div class="col-md-9">
		<select name="factory_ids[]" id="factory_ids" class="select-model form-control" value="<{if !empty($_data)}><{$_data->factory_ids()->toArray()|implode:','}><{/if}>" data-model="admin/factory" data-text="{name}" data-placeholder="请选择厂家名字" multiple="multiple"></select>
	</div>
</div>
<div class="form-group">
...
$().ready(function(){
	...
	$('#factory_ids').on('change', function(){
		$this = $(this);
		var value = $this.val();
		...
		$.POST('<{'admin/factory/data/json'|url}>', {'filters[id][in]': value}, function(json){
