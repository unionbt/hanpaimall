...
<form action="<{'admin'|url nofilter}>/<{block "name"}><{/block}>/" method="GET" class="form-bordered form-horizontal">
	...
	<div class="form-group col-sm-4">
		...
		<div class="col-md-9">
			<select name="filters[id]" id="uid" value="<{$_filters.id.equal}>" class="form-control suggest-model" data-model="admin/member" data-text="{realname}({phone})" data-term="realname" data-filters='{"role_id": "4"}' data-placeholder="请输入真实姓名查找"></select>
		</div>
	</div>
	<div class="form-group col-sm-4">
		<label class="col-md-3 control-label" for="fid">厂商</label>
		<div class="col-md-9">
			<select name="filters[fid][in][]" id="fid" class="suggest-model form-control" data-model="admin/factory" data-text="{name}" data-term="name" data-placeholder="请输入厂商名字" value="<{$_filters.fid.in|default:[]|implode:','}>" multiple="multiple"></select>
		</div>
	</div>
