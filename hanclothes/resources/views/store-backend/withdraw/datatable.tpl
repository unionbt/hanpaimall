...
<{block "filter"}>
<{include file="store-backend/withdraw/filters.inc.tpl"}>
<{/block}>
...
<{block "datatable-columns-options"}>
		var columns_options = [{'data': null, orderable: false, 'render': function (data, type, full){
			...
			return '<div class="btn-group">\
				'+(full['audit']==1?'<a href="<{''|url}>/store/withdraw/'+full['id']+'" method="delete" confirm="'+(columns_options_delete_confirm ? columns_options_delete_confirm : '')+'" data-toggle="tooltip" title="取消" class="btn btn-xs btn-danger"><i class="fa fa-times"></i></a>':'')
				+(full['audit']!=1?'无':'')+'</div>';
			}
		}];
<{/block}>
