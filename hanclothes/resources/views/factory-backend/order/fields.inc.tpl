...
<div class="form-group">
	...
	<div class="col-md-9">
		<input type="text" id="expresses_money" name="expresses_money" class="form-control" placeholder="请输入订单运费" value="<{$_data.expresses_money}>"<{if $_data.status!=\App\Order::INIT}> readonly="readonly"<{/if}>>
	</div>
</div>
...
<div class="form-group form-actions">
	...
	<{if $_data.status!=\App\Order::INIT}> 
...
(function($){
	$().ready(function(){
		$('#expresses_money').on('keyup',function(){
			var expresses_money = parseFloat($('#expresses_money').val());
			$('#total_money').html((parseFloat($('#total_money').attr('details_money'))+expresses_money).toFixed(2));
		});
	});
})(jQuery);
</script>