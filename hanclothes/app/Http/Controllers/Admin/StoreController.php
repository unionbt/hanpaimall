...
class StoreController extends Controller
{
	...
	{
		$store = new Store;
		$builder = $store->newQuery()->with('user')->join('agent_store', 'agent_store.sid', '=', 'stores.id')->groupBy('stores.id');
		...
		return $this->view('admin.store.'. ($base ? 'list' : 'datatable'));
	}
	...
	public function create()
	{
		...
		return $this->view('admin.store.create');
	}
	...
	public function edit($id)
	{
		...
		$this->_validates = $this->getScriptValidate('store.store', $keys);
		$this->_data = $store;
		return $this->view('admin.store.edit');
	}
