...
class WithdrawController extends BaseController
{
	...
	public function index(Request $request)
	{
		...
		$pagesize = $request->input('pagesize') ?: config('site.pagesize.agent-backend.'.$withdraw->getTable(), $this->site['pagesize']['common']);
		...
		return $this->view('agent-backend.withdraw.'. ($base ? 'list' : 'datatable'));
	}
	...
	public function store(Request $request)
	{
		...
		$data += ['uid' => $this->agent->getKey()];
		$withdraw = Withdraw::create($data);
		$withdraw->init();
		return $this->success('', url('agent/withdraw'));
	}
...
// 	    $amount = UserFinance::findOrNew($this->agent->getKey())->money;
// 	    //查找当前用户冻结金额
// 	    $freeze_money = Bill::whereRaw("uid = ? and is_dealt = ? and created_at>='?' and created_at<='?'",array($this->agent->getKey(),1,date("Y-m-d 00:00:00",strtotime('-7 days')),date("Y-m-d H:i:s")))->sum('value')?:0;
// 	    //可取资金
// 	    $usable_money = intval(($amount - $freeze_money)/($this->site['withdraw_counter_percent']+1));
// 	    $this->_user_money = compact('amount','freeze_money','usable_money');
...
// 		$this->_validates = $this->getScriptValidate('withdraw.store', $keys);
// 		$this->_data = $withdraw;
// 		return $this->view('agent-backend.withdraw.edit');
// 	}
