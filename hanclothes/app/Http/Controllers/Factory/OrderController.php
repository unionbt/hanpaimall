...
class OrderController extends BaseController
{
	...
	{
		$order = new Order;
		$builder = $order->newQuery()->with(['details','bills','order_express'])->where('fid', $this->factory->getKey());
		...
		return $this->view('factory-backend.order.'. ($base ? 'list' : 'datatable'));
	}
	...
	public function export(Request $request)
	{
		...
		if (empty($page)){
			...
			return $this->view('factory-backend.order.export');
		}
    ...
    public function cancel($id)
    {
        ...
        if($order->canceled())
            return $this->success('', url('factory/order'));
    }
    //发货显示
	...
	public function edit($id)
	{
		...
		$this->_data = $order;
		return $this->view('factory-backend.order.edit');
	}
